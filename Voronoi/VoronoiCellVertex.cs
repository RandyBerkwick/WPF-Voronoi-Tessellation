 ...
 Note: Unity has many serialization bugs with arrays, its just not worth the
        ...
        */
        ...
        public void AddCellWithSideEffects(VoronoiCell c)
        {
            ...
            {
                Debug.Write("Major error: you re-added a cell (" + c.name + ") to a vertex  (" + this.name + ")  that already had that cell; this suggests you've run a cel/vertex generation algorithm twice on the same VoronoiDiagram object (should never happen!)", "Error");
            }
            ...
            {
                Debug.Write("Cannot add a fourth cell  (" + c.name + ")  to a vertex  (" + this.name + ") !", "Error");
                return;
            }

            Debug.Write("  vertex  (" + this.name + ") + cell = (" + (cell1 == null ? "" : cell1.name) + ", " + (cell2 == null ? "" : cell2.name) + ", " + (cell3 == null ? "" : cell3.name) + ")  to ");
        }
        ...
        */
        ...
        public VoronoiCellEdge GetOtherEdgeContainingCell(VoronoiCellEdge currentEdge, VoronoiCell targetCell)
        {
            ...
            if (edge3 != null)
            {
                ...
                Debug.Write("Cannot find another edge from this vertex (" + this + ") that borders cell: " + targetCell, "Error");
            }
            return null;
        }
    }
